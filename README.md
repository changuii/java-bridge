...
- [x]  사용자는 위쪽, 혹은 아래쪽을 선택하여 다음 칸으로 이동한다.
  - [x]  U : 위쪽 다리로 이동한다.
  - [x]  D : 아래쪽 다리로 이동한다.
- [x]  다리의 길이를 숫자로 입력받고 생성한다.
  - [x]  0 : 아래 칸이 건널 수 있는 칸이 된다.
  - [x]  1 : 위쪽 칸이 건널 수 있는 칸이 된다.
- [x]  사용자가 다리를 건너는 것에 실패하거나, 모든 칸을 지나갈 때까지, 사용자가 이동할 칸을 입력받는다.
- [x]  이동한 칸을 건널 수 있다면 O, 건널 수 없다면 X로 표기한다.
- [x]  다리가 더이상 존재하지 않는다면 게임이 종료된다.
- [ ]  게임이 종료되면, 다음의 작업을 수행한다.
  - [x]  최종 게임 결과 출력 (이동한 내역을 출력)
  - [x]  게임의 성공 여부를 출력
  - [x]  총 시도한 횟수를 출력
- [x]  건널 수 없는 곳으로 이동하면 게임이 중단된다.
- [x]  게임이 중단되면, 다음의 작업을 수행한다.
  - [x]  게임 종료와 게임 재시도를 사용자에 입력에따라 진행된다.
